...
object mon_tab[MONSTERS] = {
	...
	{(ASLEEP|WAKENS|WANDERS|STINGS),"1d2",19,'R',10,3,12,70,0,0,0,0,0},
...
extern short halluc, blind, haste_self;
...
{
	monster->damage = mon_tab[monster->m_char - 'A'].m_damage;
}
...
/*
 * (zerogue 0.4.0) This function now allows the caller to specify whether the
 * created monster should appear confused.
 */
void create_monster( boolean mconf )
{
	...
	if (found)
	{
		...
		if( mconf )
		{
			...
			monster->moves_confused += get_rand( 1, get_rogue_level(1) ) ;
		}
	}
	else
	{
