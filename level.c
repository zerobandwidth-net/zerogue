...
extern short levitate, extra_hp, less_hp, expmod, ring_exp ; // (zerogue 0.4.1) added expmod
...
void add_exp(int e, boolean promotion)
{
	...
	if (rogue.exp_points >= level_points[rogue.exp - 1])
	{
		...
		{
			sprintf(mbuf, "Your experience level is now %d!", i);
			message(mbuf, 0);
			if (promotion)
			{
				hp = hp_raise();
				...
				if( rogue.hp_current <= rogue.hp_max )
					rogue.hp_current = rogue.hp_max ; // "Ding!"
			}
			rogue.exp = i;
...
int hp_raise(void)
{
	...
	hp = (wizard ? 10 : get_rand(3, 10)); // 1d8+2
	return(hp);
}
...
/*
 * (zerogue 0.4.1) Get a rogue's "effective level" considering actual level
 * and modifier from being hit by level drains.
 */
short get_rogue_level( boolean apply_mod )
{
	if( apply_mod )
		return rogue.exp + ring_exp + expmod ;
	else
		return rogue.exp ;
}
